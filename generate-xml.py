...
def populateStudents(root):
  ...
  for i in range(100):
    ...
    feedback.text = " ".join(random.choice(WORDS, 10))
  return student_ids
...
def populateTutorialGroups(root, student_ids):
  ...
  for i in range(10):
    ...
    name_text = "-".join(random.choice(NAMES, 2))
    name.text = name_text
    ...
    my_students = random.choice(student_ids, random.randint(2, 11))
    ...
    for i in range(random.randint(2, 5)):
      ...
      graded_students = random.choice(my_students, random.randint(1, len(my_students)))
