...
public class VersionedSuperTaClientTest {

    private final ReadOnlySuperTaClient addressBookWithAmy = new SuperTaClientBuilder().withPerson(AMY).build();
    private final ReadOnlySuperTaClient addressBookWithBob = new SuperTaClientBuilder().withPerson(BOB).build();
    private final ReadOnlySuperTaClient addressBookWithCarl = new SuperTaClientBuilder().withPerson(CARL).build();
    private final ReadOnlySuperTaClient emptyAddressBook = new SuperTaClientBuilder().build();

    @Test
    public void commit_singleAddressBook_noStatesRemovedCurrentStateSaved() {
        VersionedSuperTaClient versionedAddressBook = prepareAddressBookList(emptyAddressBook);
    ...
    public void commit_multipleAddressBookPointerAtEndOfStateList_noStatesRemovedCurrentStateSaved() {
        VersionedSuperTaClient versionedAddressBook = prepareAddressBookList(
    ...
    public void undo_singleAddressBook_throwsNoUndoableStateException() {
        ...
        assertThrows(VersionedSuperTaClient.NoUndoableStateException.class, versionedAddressBook::undo);
    }
    ...
    public void redo_singleAddressBook_throwsNoRedoableStateException() {
        ...
        assertThrows(VersionedSuperTaClient.NoRedoableStateException.class, versionedAddressBook::redo);
    }
    ...
    @Test
    public void equals() {
        VersionedSuperTaClient versionedAddressBook = prepareAddressBookList(addressBookWithAmy, addressBookWithBob);

        // same values -> returns true
        VersionedSuperTaClient copy = prepareAddressBookList(addressBookWithAmy, addressBookWithBob);
        ...
        VersionedSuperTaClient differentCurrentStatePointer = prepareAddressBookList(
     ...
     */
    private void assertAddressBookListStatus(VersionedSuperTaClient versionedAddressBook,
                                             List<ReadOnlySuperTaClient> expectedStatesBeforePointer,
                                             ReadOnlySuperTaClient expectedCurrentState,
                                             List<ReadOnlySuperTaClient> expectedStatesAfterPointer) {
        // check state currently pointing at is correct
        assertEquals(new SuperTaClient(versionedAddressBook), expectedCurrentState);
        ...
        for (ReadOnlySuperTaClient expectedAddressBook : expectedStatesBeforePointer) {
            assertEquals(expectedAddressBook, new SuperTaClient(versionedAddressBook));
            versionedAddressBook.redo();
        }

        // check states after pointer are correct
        for (ReadOnlySuperTaClient expectedAddressBook : expectedStatesAfterPointer) {
            versionedAddressBook.redo();
            assertEquals(expectedAddressBook, new SuperTaClient(versionedAddressBook));
        }
    ...
    /**
     * Creates and returns a {@code VersionedSuperTaClient} with the {@code addressBookStates} added into it, and the
     * {@code VersionedSuperTaClient#currentStatePointer} at the end of list.
     */
    private VersionedSuperTaClient prepareAddressBookList(ReadOnlySuperTaClient... addressBookStates) {
        assertFalse(addressBookStates.length == 0);

        VersionedSuperTaClient versionedAddressBook = new VersionedSuperTaClient(addressBookStates[0]);
     ...
     */
    private void shiftCurrentStatePointerLeftwards(VersionedSuperTaClient versionedAddressBook, int count) {
