...
Format: `create-tutorial-group n/[NAME] [s/STUDENT_ID] ...`

Examples:

* `create-tutorial-group n/CS1101S Studio 04A s/A0123456Y s/A1234567T s/A0132435Y`
Creates a tutorial group named `CS1101S Studio 04A` with three students with their student IDs being `A0123456Y`, `A1234567T`, and `A0132435Y`.

=== Add to Tutorial Group: `add-to-tutorial-group`

NOTE: Coming in v1.1

Adds students to a tutorial group. +
Format: `add-to-tutorial-group tg/[TUTORIAL_GROUP_ID] [s/STUDENT_ID] ...`

Examples:

* `add-to-tutorial-group tg/1 s/A1231231Y s/A2342342T`
Adds the students with student IDs `A1231231Y` and `A2342342T` to the tutorial group with an ID of `1`.
...
Creates an assignment for a specific tutorial group, identified by its ID.
Format: `create-assignment tg/[TUTORIAL_GROUP_ID] n/[NAME] m/[MAX_MARKS]`

Examples:
* `create-assignment tg/1 n/Take Home Lab 1 m/40`
Creates an assignment named `Take Home Lab 1` for the tutorial group with an ID of `1`, with the maximum marks for this assignment as `40`.
...
Format: `grade s/[STUDENT_ID] tg/[TUTORIAL_GROUP_ID] a/[ASSIGNMENT_ID] g/[GRADE]`

Examples:
* `grade s/A0123456Y tg/1 a/1 g/30`
Creates a grade with marks `30` for the student with an ID of `A0123456Y` in the tutorial group `1` for the assignment with ID `1`.
