...
.. Test case: `delete-tutorial-group id/04a` +
   Expected: The tutorial group is deleted.
.. Test case: Tutorial Group detailed view shows `[Deleted]` if user is currently viewing tutorial group details. +
   How to test:
   * Run `view-tutorial-group id/04a` first to see the detailed tutorial group view.
   * Run `delete-tutorial-group id/04a` +
   Expected: The UI shows `[Deleted]` next to the tutorial group name.
...
.. Test case: `create-assignment tg/04a n/lab1 m/40.0`
   Expected: An assignment should be made. If you are on the detailed tutorial group screen, the UI should update accordingly.
...
.. Test case: `grade tg/05a st/A0166733Y as/lab1 m/30`
   Expected: An error message showing that there is no such tutorial group should be shown.
.. Test case: `grade tg/04a st/A0123456T as/lab1 m/30`
   Expected: An error message showing that there is no such student in this tutorial group should be shown. You can also try this when there is a student in the main directory with this ID, but this student is not added to the tutorial group.
.. Test case: `grade tg/04a st/A0166733Y as/lab2 m/30`
   Expected: An error message showing that there is no such assignment should be shown.
