...
. `LogicManager` calls `parseCommand()` of `AddressBookParser`, with the user input as the argument.
. `AddressBookParser` invokes `parse()` of `DeleteAssignmentCommandParser`.
...
The following is the sequence diagram of the `delete-assignment` command:

//image::DeleteAssignmentSequenceDiagram.png[width="800"]
